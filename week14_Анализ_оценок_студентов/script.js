// Вам необходимо написать программу, которая проведёт анализ оценок студентов и выполнит несколько операций с использованием методов массивов.

// 1. Создайте пустой массив с именем `grades` и заполните массив случайными оценками от 1 до 100 для 12 студентов. Для генерации случайных оценок, используйте функцию `Math.random()`.

const grades =[];

// Заполняем массив случайными оценками от 1 до 100 для 12 студентов

for (let i = 0; i < 12; i++) {
    const randomGrade = Math.floor(Math.random() * 100) + 1; // Генерируем случайное число от 1 до 100
    grades.push(randomGrade);
  }

  console.log('Оценки студентов:', grades);

//   другой вариант:
// const grades = Array.from({ length: 12 }, () => Math.round(Math.random() * 100) + 1);
// console.log('Оценки студентов:', grades);
//  Array.from({ length: 12 }) создает новый массив длиной 12. Функция-колбэк, переданная в Array.from(), генерирует случайное число от 1 до 100

// 2. Рассчитайте и выведите средний балл студентов, используя методы массивов.

const averageGrade = grades.reduce((sum, grade) => sum + grade, 0) / grades.length;

console.log('Средний балл студентов:', averageGrade.toFixed(2));

// такой способ почему-то вычисляет среднее арифметическое 12 раз:
// function averageGrade(grades){
//     let sum = 0;
//     grades.forEach(function(num) {
//         sum += num;
//         const result = (sum / grades.length).toFixed(2)
//         console.log('Средний балл студентов:' + result)
//     });
// }

// 3. Найдите и выведите максимальный балл среди студентов, используя соответствующий метод массива.

const maxGrade = Math.max(...grades);

console.log('Максимальный балл среди студентов:', maxGrade);

// 4. Найдите и выведите минимальный балл среди студентов, используя соответствующий метод массива.

const minGrade = Math.min(...grades);

console.log('Максимальный балл среди студентов:', minGrade);

// 5. Посчитайте и выведите количество студентов, получивших положительную оценку (балл выше или равен 60). Используйте метод массива, который фильтрует оценки и оставляет только те, которые соответствуют заданному условию. Затем посчитайте количество оставшихся оценок и выведите результат.

const positiveGrade = grades.filter(num => num >= 60);
const numberPositiveGrade = positiveGrade.length
console.log ('Количество студентов, получивших положительную оценку: ' + numberPositiveGrade)

const numberOtherGrade = grades.length - numberPositiveGrade;
console.log ('Количество студентов, получивших  прочие оценки: ' + numberOtherGrade)


// 6. Посчитайте и выведите количество студентов, получивших отрицательную оценку (балл ниже 60). Используйте аналогичный подход, как в предыдущем шаге.

const negativeGrade = grades.filter(num => num <= 60);
const numberNegativeGrade = negativeGrade.length
console.log ('Количество студентов, получивших отрицательную оценку: ' + numberNegativeGrade)

// 7. Преобразуйте числовые оценки в формат буквенных оценок A/B/C/D/E, используя следующие правила:
//     - Если оценка находится в диапазоне от 80 до 100, преобразуйте её в "A"
//     - Если оценка находится в диапазоне от 60 до 79, преобразуйте её в "B"
//     - Если оценка находится в диапазоне от 40 до 59, преобразуйте её в "C"
//     - Если оценка находится в диапазоне от 20 до 39, преобразуйте её в "D"
//     - Если оценка ниже 20, преобразуйте её в "E"

// Выведите все найденные значения на экран.

// Преобразуем числовые оценки в буквенные оценки
const letterGrades = grades.map(grade => {
    if (grade >= 80 && grade <= 100) {
      return 'A';
    } else if (grade >= 60 && grade <= 79) {
      return 'B';
    } else if (grade >= 40 && grade <= 59) {
      return 'C';
    } else if (grade >= 20 && grade <= 39) {
      return 'D';
    } else {
      return 'E';
    }
  });

  console.log('Буквенные оценки студентов:', letterGrades);



