...
const myFirstArray = ['Maria', 25, true];// объявите переменную здесь
console.log(myFirstArray);

//Задание 2
//Создайте массив mySecondArray, заполните его любыми значениями. Выведите длину массива.
const mySecondArray = [1, 2, 3, 4, 5, 6]; // объявите переменную здесь
...
const myInformation = ['Nata', 15, 'December', 'Sagittarius'];
...
const moreRussianCities = ['Санкт-Петербург', 'Москва', 'Екатеринбург', ...russianCities]

console.log(moreRussianCities)
...
const sumOfFirstAndLast = (arr) => {
    if (arr.length >= 2) {
            const sum = arr[0] + arr[arr.length - 1];
            console.log('Сумма первого и последнего элементов массива:', sum);
            } else {
...
const sortAndPrint = [...randomNumbers].sort((a, b) => {
    return a-b;
})
